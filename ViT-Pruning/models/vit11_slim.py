...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, feats:int, hidden_feats:int, head:int=8, dropout:float=0.):
        ...
        self.select1 = channel_selection2(hidden_feats)

    def forward(self, x, mask=None):
        b, n, f = x.size()
        q = (self.select1(self.q(x))).view(b, n, self.head, self.hidden_feats//self.head).transpose(1,2)
        k = (self.select1(self.k(x))).view(b, n, self.head, self.hidden_feats//self.head).transpose(1,2)
        v = (self.select1(self.v(x))).view(b, n, self.head, self.hidden_feats//self.head).transpose(1,2)
