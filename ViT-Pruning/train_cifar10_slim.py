...
wandb.init(project='FineTuning_movement_cos', config={
    "batch_size": args.bs,
...
if args.resume:
    ...
    if args.select == 0:
        net.load_state_dict(torch.load("pruned_movement_cos_30_mag_{}.pth".format(str(args.percent))))
    if args.select == 1:
        net.load_state_dict(torch.load("pruned_movement_cos_30_magfisher_{}.pth".format(str(args.percent))))
    if args.select == 2:
        net.load_state_dict(torch.load("pruned_movement_cos_30_fisher_{}.pth".format(str(args.percent))))
    if args.select == 3:
        net.load_state_dict(torch.load("pruned_movement_cos_30_taylor_{}.pth".format(str(args.percent))))
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        if args.select == 0:
          torch.save(state, './checkpoint/'+args.net+'-4_movement_cos_30-ckpt_mag_{}_.t7'.format(str(args.percent)))
        if args.select == 1:
          torch.save(state, './checkpoint/'+args.net+'-4_movement_cos_30-ckpt_magfisher_{}_.t7'.format(str(args.percent)))
        if args.select == 2:
          torch.save(state, './checkpoint/'+args.net+'-4_movement_cos_30-ckpt_fisher_{}_.t7'.format(str(args.percent)))
        if args.select == 3:
          torch.save(state, './checkpoint/'+args.net+'-4_movement_cos_30-ckpt_taylor_{}_.t7'.format(str(args.percent)))
        best_acc = acc
