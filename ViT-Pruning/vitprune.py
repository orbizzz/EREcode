...
model_path = "/content/drive/MyDrive/ViT-cifar10-pruning/checkpoint/vit-4-ckpt_movement_cos_30.t7"
    ...
    for m in model.modules():
        ...
        if select == 3:
            if isinstance(m, channel_selection):
                ...
                for i in range(size):
                    bn[index + i] = (-0.5 * weight_clone[i] * weight_clone[i] / hessian_inverse[i][i] 
                                    - 0.5 * inverse_grad[i] * inverse_grad[i] / hessian_inverse[i][i] 
                                    + grad_clone[i] * inverse_grad[i] / hessian_inverse[i][i])
                index += size
...
if select == 0:
    model_name = "pruned_movement_cos_30_mag_{}.pth".format(percent)
    torch.save(newmodel.state_dict(), model_name)
if select == 1:
    model_name = "pruned_movement_cos_30_magfisher_{}.pth".format(percent)
    torch.save(newmodel.state_dict(), model_name)
if select == 2:
    model_name = "pruned_movement_cos_30_fisher_{}.pth".format(percent)
    torch.save(newmodel.state_dict(), model_name)
if select == 3:
    model_name = "pruned_movement_cos_30_taylor_{}.pth".format(percent)
    torch.save(newmodel.state_dict(), model_name)
