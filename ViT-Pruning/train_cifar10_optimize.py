...
# wandb.init(project='InitialLearning_l1movement', config={
#     "batch_size": args.bs,
...
sweep_id = wandb.sweep(sweep_config, project="pytorch-vit-sweep_2")
...
#     checkpoint = torch.load('./checkpoint/{}-4-ckpt_l1movement.t7'.format(args.net))
...
def sparse_selection(s_1, s_2):
    ...
    for m in net.modules():
        ...
        if isinstance(m, channel_selection):
            m.indexes.grad.data.add_(s_1*torch.sign(m.indexes.data))
            grad_clone = m.indexes.grad.clone()
            grad_clone.data.mul_(-s_2*m.indexes.data)
             # L1
...
def train(config):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        #         hes_cal(loss)
        loss.backward()
        sparse_selection(config.s_1, config.s_2)
        optimizer.step()
        # sparse_selection_2()
...
def test(config):
    ...
    #         os.mkdir('checkpoint')
    #     torch.save(state, './checkpoint/'+args.net+'-{}-ckpt_l1movement.t7'.format(args.patch))
    #     best_acc = acc
...
def train_iter(config=None):
    with wandb.init(config=config):
        ...
        print('==> Resuming from checkpoint..')
        assert os.path.isdir('checkpoint'), 'Error: no checkpoint directory found!'
        checkpoint = torch.load('./checkpoint/vit-4-ckpt_l1movement_cos_40.t7')
        net.load_state_dict(checkpoint['net'])
        # best_acc = checkpoint['acc']
        # start_epoch = checkpoint['epoch']
        ...
        for epoch in range(config.epochs):
            trainloss = train(config)
            val_loss, acc = test(config)
            wandb.log({"loss": val_loss, "epoch": epoch, "learning_late": optimizer.param_groups[0]["lr"]})
...
if __name__ == "__main__":
    wandb.agent(sweep_id, train_iter, count=60)  
