...
model_path = "/content/drive/MyDrive/ViT-cifar10-pruning/checkpoint/vit11-4-ckpt_l1magnitude_cos_999.t7"
            ...
            if k in [12, 31, 50, 69, 88, 107, 126]:
                ...
                if select == 2:
                    while (torch.sum(mask)%8 !=0 or torch.sum(mask) < 8):                       # heads
                        thre_ = thre_ - 0.0000000002
                        mask = weight_copy.gt(thre_).float().cuda()
                else:
                    while (torch.sum(mask)%8 !=0 or torch.sum(mask) < 8):                       # heads
                        thre_ = thre_ - 0.000001
                        mask = weight_copy.gt(thre_).float().cuda()
            else:
                if select ==  0:
                    weight_copy = m.indexes.data.abs().clone() # magnitude
...
model_num_parameters = sum([param.nelement() for param in model.parameters() if param.requires_grad])
newmodel_num_parameters = sum([param.nelement() for param in newmodel.parameters() if param.requires_grad])
...
if select == 0:
    model_name = "pruned_l1magnitude_cos_999_mag_{}.pth".format(percent)
    torch.save(newmodel.state_dict(), model_name)
if select == 1:
    model_name = "pruned_l1magnitude_cos_999_magfisher_{}.pth".format(percent)
    torch.save(newmodel.state_dict(), model_name)
if select == 2:
    model_name = "pruned_l1magnitude_cos_999_fisher_{}.pth".format(percent)
    torch.save(newmodel.state_dict(), model_name)
if select == 3:
    model_name = "pruned_l1magnitude_cos_999_taylor_{}.pth".format(percent)
    torch.save(newmodel.state_dict(), model_name)
